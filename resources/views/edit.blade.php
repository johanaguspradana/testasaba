            ...
            <div class="modal-body">
                <form id="formedit">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="editTepung" name="tepung" required>
                    </div>
                    <div class="form-group">
                        <label for="editCoklat">Coklat Padat</label>
                        <input type="text" class="form-control" id="editCoklat" name="coklat" required>
                    </div>
...
<script>
    ...
    $('#editModal').on('show.bs.modal', function (event) {
        ...
        var bahangula = button.data('gula') // Extract info from data-* attributes
        var bahantepung = button.data('tepung') // Extract info from data-* attributes
        var bahancoklat = button.data('coklat') // Extract info from data-* attributes
        // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
        // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
        var modal = $(this)
        modal.find('.modal-title').text('New message to ')
        modal.find('.modal-body input[name=id]').val(bahan)
        modal.find('.modal-body input[name=gula]').val(bahangula)
        modal.find('.modal-body input[name=tepung]').val(bahantepung)
        modal.find('.modal-body input[name=coklat]').val(bahancoklat)
    });
